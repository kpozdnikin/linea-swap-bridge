...
export class TransactionLoopring extends Transaction {
  ...
  private async checkLoopringAccountKey(fromAddress: string) {
    ...
    if (!account.accountInfo) {
      const accountResult = await exchangeApi.getAccount({ owner: fromAddress });
      ...
      const options = {
        ...
        keySeed:
          accountInfo.keySeed && accountInfo.keySeed !== ""
            ? accountInfo.keySeed
            : GlobalAPI.KEY_MESSAGE.replace("${exchangeAddress}", exchangeInfo.exchangeAddress).replace(
                "${nonce}",
                (accountInfo.nonce - 1).toString(),
              ),
        walletType: ConnectorNames.WalletLink,
        chainId:
          ChainValidator.loopring(this.chainId) == ChainValidatorTypes.Mainnet ? ChainId.MAINNET : ChainId.GOERLI,
      };
  ...
  public async transfer(options: TransactionTransferOptions) {
    if (!options.fromAddress) {
      throw new Error("Sorry, miss param [fromAddress]");
    }
    const networkId = ChainValidator.loopring(this.chainId) == ChainValidatorTypes.Mainnet ? 1 : 5;
    const exchangeApi = new ExchangeAPI({ chainId: networkId });
    const userApi = new UserAPI({ chainId: networkId });
    ...
    return await userApi.submitInternalTransfer({
      ...
      web3: this.web3,
      chainId: ChainValidator.loopring(this.chainId) === ChainValidatorTypes.Mainnet ? ChainId.MAINNET : ChainId.GOERLI,
      walletType: ConnectorNames.WalletLink,
