...
class IMXListen {
  ...
  private tickerTimer: NodeJS.Timer | undefined;
   ...
   * @returns
   */
  private async doTransfer(transfer: any, retryCount = 0): Promise<void> {
    ...
    if (retryCount > 0) {
      try {
        ...
        console.error(`Get imx transaction [${transaction_id}] failed: ${err}, retryCount: ${retryCount}`);
        ...
        this.doTransfer(transfer, (retryCount += 1));
    ...
    const isConfirmed =
      equalsIgnoreCase(txreceipt_status, "confirmed") || equalsIgnoreCase(txreceipt_status, "success");
...
const factorys: Record<string, any> = {};
/**
 *
 * @param chainId
 * @param receiver
 * @returns
 */
export function factoryIMXListen(chainId: number, receiver: string | undefined = undefined, isFirstTicker = true) {
