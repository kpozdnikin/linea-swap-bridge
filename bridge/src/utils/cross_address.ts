...
export class CrossAddress {
  ...
  private providerNetworkId: number | undefined;
  ...
  constructor(signer: Signer, orbiterChainId = 5) {
    this.contractAddress = config.contracts.crossAddress[orbiterChainId] as string;
   ...
   * @return
   */
  async transfer(to: string, amount: ethers.BigNumberish, ext: CrossAddressExt | undefined = undefined) {
    await this.checkNetworkId();

    if (ext && !CrossAddressExtTypes[ext.type as "0x01" | "0x02"]) {
    ...
    const contract = new ethers.Contract(this.contractAddress, config.abis.crossAddress, this.signer);
  ...
  async transferERC20(
    ...
    const contract = new ethers.Contract(this.contractAddress, config.abis.crossAddress, this.signer);
    const extHex = CrossAddress.encodeExt(ext);
    return <TransactionResponse>await contract.transferERC20(tokenAddress, to, amount.toHexString(), extHex);
  }
  ...
  static parseTransferInput(input: string): {
    ...
    const [to, ext] = utils.defaultAbiCoder.decode(["address", "bytes"], utils.hexDataSlice(input, 4));
