...
const DYDX_MAKERS: Record<string, { starkKey: string; positionId: string }> = {
  // Testnet
...
const DYDX_CLIENTS: Record<string, any> = {};
const DYDX_ACCOUNTS: Record<string, any> = {};
...
export class DydxHelper {
   ...
   * @returns
   */
  async getDydxClient(ethereumAddress = "", alwaysNew = false, alwaysDeriveStarkKey = false): Promise<DydxClient> {
    ...
    if (clientOld && !alwaysNew) {
      if (alwaysDeriveStarkKey && ethereumAddress) {
        ...
        clientOld.starkPrivateKey = await clientOld.onboarding.deriveStarkKey(ethereumAddress, this.signingMethod);
      }
    ...
    if (ethereumAddress && this.web3) {
      ...
      if (userExists.exists) {
        if (alwaysDeriveStarkKey) {
          client.starkPrivateKey = await client.onboarding.deriveStarkKey(ethereumAddress, this.signingMethod);
        }
  ...
  static toTransaction(transfer: TransferResponseObject, ethereumAddress: string) {
    ...
    const transaction = {
      ...
      from: "",
      to: "",
      value: ethers.utils.parseUnits(isTransferIn ? transfer.creditAmount : transfer.debitAmount, 6).toString(), // Only usdc
