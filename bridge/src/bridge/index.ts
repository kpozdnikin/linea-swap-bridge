...
export class Bridge {
  ...
  private makerList: typeof makerList_mainnet | typeof makerList_testnet | undefined;
  ...
  private async ensureStarkAccount(
    ...
    // dYdX
    let dydxChainId = 0;
    if (ChainValidator.dydx((dydxChainId = fromChain.id)) || ChainValidator.dydx((dydxChainId = toChain.id))) {
    ...
    for (const item of makerList) {
      this.expandMakerInfo(item).forEach((makerInfo) => {
        ...
        const findIndexToken = tokens.findIndex(
          (_token) =>
            equalsIgnoreCase(_token.address, makerInfo.fromTokenAddress) && _token.chainId == makerInfo.fromChainId,
        );
   ...
   * @returns
   */
  public async getTargetMakerInfo(token: BridgeToken, fromChain: BridgeChain, toChain: BridgeChain) {
    const makerList = await this.getMakerList();
